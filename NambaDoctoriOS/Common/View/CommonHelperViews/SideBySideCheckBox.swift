...
struct ThreeItemCheckBox: View {
    ...
    var body: some View {
        HStack {
            ...
            HStack{
                Image(systemName: isChecked == title3 ? "smallcircle.fill.circle": "circle")
                    .foregroundColor(isChecked == title3 ? .blue : .gray)
                Text(title3)
                    .font(.system(size: 14))
            }.padding(.leading)
            .onTapGesture {
                isChecked = title3
                self.checkBoxDelegate?.itemChecked(value: title3)
            }
